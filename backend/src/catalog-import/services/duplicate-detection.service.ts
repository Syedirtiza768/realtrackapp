  ...
  async checkDuplicateBatch(
    ...
    if (skuEntries.length > 0) {
      ...
      if (scope?.organizationId)
        existingBySkuQuery.andWhere(
          'p.organization_id = :scopeOrganizationId',
          { scopeOrganizationId: scope.organizationId },
        );
      if (scope?.vertical)
        existingBySkuQuery.andWhere(
          scope.vertical === 'automotive'
            ? '(p.vertical = :scopeVertical OR p.vertical IS NULL)'
            : 'p.vertical = :scopeVertical',
