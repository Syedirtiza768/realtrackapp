...
} from './entities/user-store-assignment.entity.js';
...
export class StoreAccessService {
   ...
   */
  async getStoreIdsAtLevel(
    user: User,
    minLevel: StoreAccessLevel,
  ): Promise<Set<string>> {
    if (user.storeAccessAll) {
      const all = await this.storeRepo.find({ select: ['id'] });
      return new Set(all.map((s) => s.id));
    ...
    const levelRank: Record<StoreAccessLevel, number> = {
      view: 0,
      operate: 1,
      admin: 2,
    };
    ...
    return new Set(
      assignments
        .filter((a) => levelRank[a.accessLevel] >= minRank)
        .map((a) => a.storeId),
    );
  }
  ...
  async assertStoreAccess(
    user: User,
    storeId: string,
    minLevel: StoreAccessLevel = 'view',
  ): Promise<void> {
  ...
  static readonly UNSCOPED_LISTINGS_SQL = `r.id NOT IN (SELECT lci.listing_id FROM listing_channel_instances lci)`;

  /** Listings in the user's stores plus unscoped catalog listings. */
  static readonly SCOPED_LISTINGS_SQL = `(r.id IN (SELECT lci.listing_id FROM listing_channel_instances lci WHERE lci.store_id IN (:...storeIds))
