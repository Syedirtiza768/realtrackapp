...
export class EbaySellingMetadataService {
  ...
  async getCategoryPolicies(
    ...
    const [conditions, structures] = await Promise.all([
      http.get(
        `/marketplace/${encodeURIComponent(marketplaceId)}/get_item_condition_policies`,
        {
          params: { category_ids: categoryId },
        },
      ),
      http.get(
        `/marketplace/${encodeURIComponent(marketplaceId)}/get_listing_structure_policies`,
        {
          params: { category_ids: categoryId },
        },
      ),
    ...
    const conditionPolicies = Array.isArray(
      conditions.data?.itemConditionPolicies,
    )
      ? conditions.data.itemConditionPolicies
      : [];
    const categoryPolicy =
      conditionPolicies.find(
        (policy: Record<string, unknown>) =>
          String(policy.categoryId ?? '') === categoryId,
      ) ?? conditionPolicies[0];
    const supportedConditions = Array.isArray(categoryPolicy?.conditionIdValues)
      ? categoryPolicy.conditionIdValues
          .map((value: Record<string, unknown>) =>
            String(value.conditionId ?? value.id ?? ''),
          )
          .filter(Boolean)
      : [];

    const structurePolicies = Array.isArray(
      structures.data?.listingStructurePolicies,
    )
      ? structures.data.listingStructurePolicies
      : [];
    const structure =
      structurePolicies.find(
        (policy: Record<string, unknown>) =>
          String(policy.categoryId ?? '') === categoryId,
      ) ?? structurePolicies[0];
    const supportsVariations =
      typeof structure?.variationsSupported === 'boolean'
