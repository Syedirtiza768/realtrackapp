...
export class ListingBuilderService {
  ...
  async build(params: {
    ...
    /** Connected-store policy/profile values selected at publish time. */
    ...
    const vertical: ProductVertical =
      params.vertical ??
      snapshot.vertical ??
      catalogProduct?.vertical ??
      listingRecord?.vertical ??
      'automotive';
    const projection =
      vertical !== 'automotive' && this.verticals
    ...
    const parsedFitmentRows = (
      vertical === 'automotive' ? (catalogProduct?.fitmentData ?? []) : []
    )
    ...
    const title =
      vertical === 'automotive'
        ? titleResult.title
        : ov?.titleOverride?.trim() || projection?.title || snapshot.title;
    ...
    if (vertical === 'automotive') {
      const safeCategory = resolveSafePublishCategory({
        categoryId,
        categoryName:
          catalogProduct?.categoryName ?? listingRecord?.categoryName,
        listingText: [
          snapshot.title,
          snapshot.partType,
    ...
    const condition: EbayConditionEnum =
      vertical === 'automotive'
    ...
    // A queued publish may carry the connected-store selection made in the
    // catalog modal. It takes precedence over a stale saved override while
    // still falling back to the durable per-product override row.
    const po = {
      ...(ov?.policyOverrides ?? {}),
    ...
    const overrideProfileName = (key: string): string | undefined =>
      typeof po[key] === 'string' && (po[key] as string).trim()
        ? (po[key] as string).trim()
        : undefined;
    const requestedFulfillmentPolicyName =
      overrideProfileName('requestedFulfillmentPolicyName') ||
      listingRecord?.shippingProfileName?.trim() ||
      catalogProduct?.shippingProfile?.trim() ||
      undefined;
    const requestedReturnPolicyName =
      overrideProfileName('requestedReturnPolicyName') ||
      listingRecord?.returnProfileName?.trim() ||
      catalogProduct?.returnProfile?.trim() ||
      undefined;
    const requestedPaymentPolicyName =
      overrideProfileName('requestedPaymentPolicyName') ||
        ...
        vertical === 'automotive'
          ? snapshot.conditionLabel?.trim() ||
            (isUsedEbayCondition(condition)
              ...
              : undefined)
          : projection?.conditionDescription ||
            snapshot.conditionLabel?.trim() ||
            undefined,
      listingDuration: 'GTC',
