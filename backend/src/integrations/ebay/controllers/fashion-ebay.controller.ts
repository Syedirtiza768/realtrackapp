...
export class FashionEbayController {
  ...
  async start(@Body() dto: EbayOAuthStartDto, @CurrentUser() user: User) {
    ...
    if (dto.internalStoreId)
      throw new BadRequestException(
        'Fashion connections must create a dedicated seller store',
      );
    return this.oauth.startOAuth({
      ...
      accountDisplayName:
        dto.accountDisplayName?.trim() || 'Fashion eBay store',
      vertical: 'fashion',
    });
  }
  ...
  async publishBulk(
    ...
    await this.verticals.assertFeatureEnabled();
    const listingIds = [...new Set(dto.listingIds)];
    ...
    if (products.length !== listingIds.length)
      throw new BadRequestException(
        'One or more selected Fashion catalog products were not found',
      );
    for (const product of products)
      await this.fashion.assertApproved(product.id, org.organizationId);
    ...
    for (const storeId of storeIds) {
      ...
      if (!account?.primaryStore)
        throw new BadRequestException(
          'One or more selected stores are not active Fashion eBay stores',
        );
      if (!accessible.has(storeId))
        throw new ForbiddenException(
          'You do not have access to one or more selected Fashion stores',
        );
      await this.storeAccess.assertStoreAccess(user, storeId, 'operate');
      const config = this.verticals.getStoreConfig(account.primaryStore);
      if (
        config.enabledVerticals.length !== 1 ||
        config.enabledVerticals[0] !== 'fashion'
        ...
        throw new BadRequestException(
          'Fashion publishing requires dedicated Fashion stores',
        );
    }
  ...
  async channels(
    ...
    for (const row of rows) {
      if (
        row.catalogProduct?.vertical !== 'fashion' ||
        row.catalogProduct.organizationId !== org.organizationId
      )
        continue;
      try {
        const account = await this.fashion.account(
          row.ebayAccountId,
          org.organizationId,
        );
  ...
  private async authorizedAccount(
    ...
    await this.storeAccess.assertStoreAccess(
      user,
      account.primaryStoreId,
      level,
    );
  ...
  private marketplace(account: ConnectedEbayAccount, requested?: string) {
    ...
    if (
      ...
      !account.marketplaces.some(
        (mp) => mp.enabled && mp.marketplaceId === marketplace,
      )
    )
      throw new BadRequestException(
        'Marketplace is not enabled for this Fashion seller account',
      );
    return marketplace;
  }
  ...
  private async assertPublishable(dto: EbayValidateDto, user: User) {
    ...
    if (!dto.targets.length || dto.targets.length > 20)
      throw new BadRequestException('Choose 1-20 Fashion publishing targets');
    for (const target of dto.targets) {
      const { account } = await this.authorizedAccount(
        user,
  ...
  private async authorizedJob(user: User, id: string, organizationId?: string) {
    ...
    for (const target of targets) {
      if (
        ...
        !target.ebayAccountId ||
        (target.vertical && target.vertical !== 'fashion')
      )
        throw new NotFoundException('Fashion job not found');
      if (
