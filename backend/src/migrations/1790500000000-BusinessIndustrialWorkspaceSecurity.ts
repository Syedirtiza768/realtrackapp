...
export class BusinessIndustrialWorkspaceSecurity1790500000000 implements MigrationInterface {
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    `);
    await queryRunner.query(
      `CREATE INDEX IF NOT EXISTS "idx_bi_review_org_status" ON "business_industrial_reviews" ("organization_id", "status")`,
    );
    ...
    `);
    await queryRunner.query(
      `CREATE INDEX IF NOT EXISTS "idx_bi_unit_product_status" ON "business_industrial_units" ("catalog_product_id", "status")`,
    );
    ...
    `);
    await queryRunner.query(
      `CREATE INDEX IF NOT EXISTS "idx_bi_incident_org_status" ON "business_industrial_incidents" ("organization_id", "status")`,
    );
    await queryRunner.query(
      `CREATE INDEX IF NOT EXISTS "idx_bi_incident_product" ON "business_industrial_incidents" ("catalog_product_id")`,
    );
  }

  public async down(queryRunner: QueryRunner): Promise<void> {
    await queryRunner.query(
      `DROP TABLE IF EXISTS "business_industrial_incidents"`,
    );
    await queryRunner.query(`DROP TABLE IF EXISTS "business_industrial_units"`);
    await queryRunner.query(
      `DROP TABLE IF EXISTS "business_industrial_reviews"`,
    );
  }
}
