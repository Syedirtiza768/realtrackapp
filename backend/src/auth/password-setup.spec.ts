...
describe('First-login password setup HTTP flow', () => {
  ...
  const repo = {
    findOne: jest.fn(({ where }: { where: Partial<User> }) => {
      if (
        ...
        (where.active !== undefined && where.active !== user.active)
      )
  ...
  beforeAll(async () => {
    ...
    const module = await Test.createTestingModule({
      imports: [
        PassportModule,
        JwtModule.register({ secret: 'password-setup-test-signing-key' }),
      ],
      ...
      providers: [
        ...
        { provide: getRepositoryToken(User), useValue: repo },
        {
          ...
          useValue: {
            get: (key: string) =>
              key === 'JWT_SECRET'
                ? 'password-setup-test-signing-key'
                : 'false',
          },
        },
        {
        ...
        { provide: AuthAuditService, useValue: { log: audit } },
        {
          ...
          useValue: {
            ...
            revokeBearerToken: jest.fn().mockResolvedValue(undefined),
          },
        },
        {
          provide: RbacService,
          useValue: {
            getAuthProfile: jest.fn((value: User) => ({
              id: value.id,
              email: value.email,
            })),
            ...
            getPermissionKeysForUser: jest
              .fn()
              .mockResolvedValue(new Set(['fashion.access'])),
          },
        },
  ...
  beforeEach(() => {
    ...
    update.mockImplementation(
      (where: string | Partial<User>, changes: Partial<User>) => {
        if (
          typeof where !== 'string' &&
          where.passwordHash &&
  ...
  it.each([
    '/api/password-test/ordinary',
    '/api/password-test/fashion',
    '/api/auth/organizations',
  ])('blocks %s even with a valid JWT', async (path) => {
  ...
  it('rejects an incorrect current password without clearing the flag', async () => {
    await request(app.getHttpServer())
      .patch('/api/auth/change-password')
      .auth(token, { type: 'bearer' })
      .send({ currentPassword: 'incorrect', newPassword })
      .expect(400);
  ...
  it.each(['short', 'a'.repeat(73), '😀'.repeat(19), currentPassword])(
    'rejects an invalid or reused new password (%#)',
    async (candidate) => {
      await request(app.getHttpServer())
        .patch('/api/auth/change-password')
        .auth(token, { type: 'bearer' })
        .send({ currentPassword, newPassword: candidate })
        .expect(400);
      expect(update).not.toHaveBeenCalled();
    },
  );
  ...
  it('atomically changes the hash and flag, then permits the same JWT on the next request', async () => {
    ...
    expect(await bcrypt.compare(currentPassword, user.passwordHash)).toBe(
      false,
    );
    expect(update).toHaveBeenCalledWith(
      { id: user.id, active: true, passwordHash: oldHash },
      { passwordHash: user.passwordHash, passwordChangeRequired: false },
    );
    await request(app.getHttpServer())
      .get('/api/password-test/fashion')
      .auth(token, { type: 'bearer' })
      .expect(200);
  ...
  it('does not force unflagged non-Fashion accounts to change passwords', async () => {
    ...
    await request(app.getHttpServer())
      .get('/api/password-test/ordinary')
      .auth(token, { type: 'bearer' })
      .expect(200);
  });
