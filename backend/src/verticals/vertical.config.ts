...
export const VERTICAL_PROFILES: Record<ProductVertical, VerticalProfile> = {
  ...
  business_industrial: {
    ...
    attributeFields: [
      ...
      { key: 'includedAccessories', label: 'Included accessories', input: 'textarea' },
    ],
  ...
  fashion: {
    ...
    attributeFields: [
      ...
      { key: 'conditionDetails', label: 'Condition details', input: 'textarea' },
    ],
    supportsFitment: false,
    supportsVariations: true,
  },
...
export function normalizeVerticalConfig(value: unknown): VerticalConfig {
  ...
  const enabled = Array.isArray(raw.enabledVerticals)
    ...
    : [];
  const enabledVerticals: ProductVertical[] = enabled.length > 0 ? [...new Set(enabled)] : [DEFAULT_PRODUCT_VERTICAL];
...
function normalizeAttributeKey(key: string): string {
  return key.trim().replace(/[^a-zA-Z0-9]+(.)/g, (_match, ch: string) => ch.toUpperCase());
}
...
export function validateVerticalAttributes(
  ...
  if (typeof value !== 'object' || Array.isArray(value)) {
    return { attributes: {}, errors: ['Vertical attributes must be an object'] };
  }
  ...
  for (const [rawKey, rawValue] of Object.entries(value as Record<string, unknown>)) {
    ...
    if (Array.isArray(rawValue) && rawValue.every((item) => typeof item === 'string')) {
      attributes[key] = rawValue as string[];
      continue;
    }
    errors.push(`${key} must be a string, number, boolean, or string array`);
  }

  if (vertical !== 'automotive' && ('fitment' in attributes || 'vin' in attributes)) {
    errors.push(`${vertical} products cannot contain automotive fitment or VIN attributes`);
  }
...
export function attributesFromImportRow(
  ...
  const keys = vertical === 'fashion'
    ? ['brand', 'department', 'size', 'sizeSystem', 'color', 'material', 'measurements', 'conditionDetails']
    : vertical === 'business_industrial'
      ? ['manufacturer', 'model', 'mpn', 'voltage', 'power', 'dimensions', 'capacity', 'testingStatus', 'includedAccessories']
      : [];
