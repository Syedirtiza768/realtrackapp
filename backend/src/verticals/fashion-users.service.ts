...
export class FashionUsersService {
  ...
  async list(user: User, organizationId?: string) {
    ...
    for (const member of members) {
      ...
      if (
        FASHION_ROLES.includes(profile.roleSlug) &&
        profile.permissions.every((key) => key.startsWith('fashion.'))
      )
        result.push(
      ...
      saved = await this.dataSource.transaction(async (manager) => {
        ...
        if (
          ...
          throw new ConflictException(
            'Email already registered; existing accounts cannot be claimed by Fashion admins',
          );
  ...
  private async safeRole(manager: EntityManager, slug: string) {
    if (!FASHION_ROLES.includes(slug))
      throw new ForbiddenException('Only Fashion roles may be assigned');
    ...
    if (
      !role ||
      !keys.includes('fashion.access') ||
      keys.some((key) => !key?.startsWith('fashion.'))
    )
      throw new ForbiddenException(
        'Role is missing or grants access outside Fashion',
      );
    return role;
  }
  ...
  private async mutate(
    ...
    if (actor.id === id)
      throw new ForbiddenException(
        'Fashion administrators cannot change their own access',
      );
    const org = await this.organizations.resolveOrganizationId(
      actor.id,
      organizationId,
    ...
    const result = await this.dataSource.transaction(async (manager) => {
      ...
      if (
        ...
        throw new NotFoundException('Fashion user not found');
      if (
        members.length !== 1 ||
        target.role !== 'user' ||
        ...
        throw new ForbiddenException(
          'Shared or platform accounts cannot be changed by Fashion admins',
        );
      ...
      if (
        !assignments.length ||
        assignments.some((row) => !FASHION_ROLES.includes(row.role?.slug))
      )
        throw new ForbiddenException(
          'Only exclusively Fashion accounts can be changed',
        );
      ...
      if (
        storeAssignments.some(
          (row) =>
            ...
            !this.isDedicatedFashion(row.store),
        )
      )
        throw new ForbiddenException(
          'User has stores outside this Fashion workspace',
        );
  ...
  private async validateStores(
    ...
    for (const id of ids) {
      if (
        !available.some(
          (store) => store.id === id && this.isDedicatedFashion(store),
        )
      )
        throw new ForbiddenException(
          'Store is not dedicated to this Fashion workspace',
        );
  ...
  private async summary(
    ...
    const allowed = new Set(
      (await this.verticals.listStoresForOrganization(organizationId))
        .filter((store) => this.isDedicatedFashion(store))
        .map((store) => store.id),
    );
