...
export class VerticalsService {
  ...
  private readonly metadataCache = new Map<
    string,
    { value: VerticalCategoryMetadata; expiresAt: number }
  >();
  ...
  constructor(
    ...
    @InjectRepository(ProductMarketplaceCategory)
    private readonly categoryRepo: Repository<ProductMarketplaceCategory>,
    @InjectRepository(BusinessIndustrialReview)
    private readonly businessIndustrialReviewRepo: Repository<BusinessIndustrialReview>,
  ) {}
  ...
  async assertIntakeVerticalEnabled(
  ...
  getStoreConfig(store: Store): VerticalConfig {
    return normalizeVerticalConfig(
      store.verticalConfig ?? DEFAULT_VERTICAL_CONFIG,
    );
  }

  async listStoresForOrganization(
    organizationId: string,
  ): Promise<Array<Store & { verticalConfig: VerticalConfig }>> {
    const stores = await this.storeRepo
      ...
      .leftJoinAndMapOne(
        ...
        'account',
        'account.primary_store_id = store.id',
      )
      .where(
        'store.organization_id = :organizationId OR account.organization_id = :organizationId',
        { organizationId },
      )
    ...
    return stores.map((store) => ({
      ...store,
      verticalConfig: this.getStoreConfig(store),
    }));
  }

  async getStoreConfigForOrganization(
    storeId: string,
    organizationId: string,
  ...
  async updateFashionStoreConfig(
    ...
    const enabledVerticals = enabled
      ? [
          ...new Set([
            ...current.enabledVerticals,
            'fashion' as ProductVertical,
          ]),
        ]
  ...
  async updateStoreConfig(
    ...
    const newVerticals = next.enabledVerticals.filter(
      (v) => v !== DEFAULT_PRODUCT_VERTICAL,
    );
    if (
      newVerticals.length ||
      next.defaultVertical !== DEFAULT_PRODUCT_VERTICAL
    ) {
      await this.assertFeatureEnabled();
    }
    if (!next.enabledVerticals.includes(next.defaultVertical)) {
      throw new BadRequestException(
        'The store default vertical must be enabled for that store',
      );
    }
    store.verticalConfig = next;
    await this.storeRepo.save(store);
  ...
  resolveVertical(
    ...
    if (frozenVertical && isProductVertical(frozenVertical))
      return frozenVertical;
    if (product?.vertical && isProductVertical(product.vertical))
      return product.vertical;
    if (listing?.vertical && isProductVertical(listing.vertical))
      return listing.vertical;
  ...
  normalizeIntakeVertical(value: unknown): ProductVertical {
    ...
    if (
      value != null &&
      typeof value === 'string' &&
      !isProductVertical(value) &&
      vertical === DEFAULT_PRODUCT_VERTICAL
    ) {
      const normalized = value.trim().toLowerCase().replace(/[ -]+/g, '_');
      if (!['auto', 'motors', 'automotive'].includes(normalized)) {
        throw new BadRequestException(
          `Unsupported product vertical "${value}"`,
        );
      }
    }
    return vertical;
  ...
  async buildPublishProjection(params: {
    ...
    const warnings = [
      ...productAttributes.errors.map(
        (error) => `Invalid vertical attributes: ${error}`,
      ),
    ];
    ...
    if (params.vertical === 'business_industrial') {
      ...
      for (const error of businessIndustrial.errors) {
        blockingErrors.push(
          `Invalid Business & Industrial attributes: ${error}`,
        );
      }
      ...
      const review = await this.businessIndustrialReviewRepo.findOne({
        where: {
          organizationId: productIdentity.organizationId!,
      ...
      if (!shippingMode)
        blockingErrors.push(
          'shippingMode is required for Business & Industrial publishing',
        );
      if (!attributes.dispatchLocation)
        blockingErrors.push(
          'dispatchLocation is required for Business & Industrial publishing',
        );
      if (!attributes.shippingCoverage)
        blockingErrors.push(
          'shippingCoverage is required for Business & Industrial publishing',
        );
      if (shippingMode === 'parcel' || shippingMode === 'freight') {
        for (const field of [
          ...
          if (attributes[field] === undefined || attributes[field] === '')
            blockingErrors.push(
              `${field} is required for ${shippingMode} shipping`,
            );
        }
      }
      if (shippingMode === 'freight' && !attributes.freightLoadingFacilities) {
        blockingErrors.push(
          'freightLoadingFacilities is required for freight shipping',
        );
      }
    }
    ...
    const categoryId =
      ...
      params.product.categoryId?.trim() ||
      '';
    const metadata = categoryId
      ? await this.getCategoryMetadata(
          params.storeId,
          params.marketplaceId,
          categoryId,
        ).catch((error: unknown) => {
          blockingErrors.push(
            `Category metadata could not be validated for ${categoryId}. Refresh eBay metadata and try again: ${error instanceof Error ? error.message : String(error)}`,
          );
          return null;
        })
      : null;
    ...
    if (metadata?.supportedConditions.length) {
      if (!conditionId) {
        ...
        blockingErrors.push(
          'eBay condition ' +
            conditionId +
            ' is not supported by category ' +
            categoryId,
        );
      }
    }
    const aspects = this.buildAspects(
      params.product,
      params.listing,
    ...
    if (metadata) {
      for (const aspect of metadata.aspects) {
        ...
        const normalize = (key: string) =>
          key
            .trim()
            .replace(/[^a-zA-Z0-9]+(.)/g, (_match, ch: string) =>
              ch.toUpperCase(),
            )
            .toLowerCase();
        ...
        if (typeof maxValues === 'number' && values.length > maxValues) {
          blockingErrors.push(
            `eBay aspect "${name}" allows at most ${maxValues} value(s)`,
          );
        }
      }
    }
    ...
    if (
      !params.product.imageUrls?.length &&
      !params.listing?.itemPhotoUrl?.trim()
    )
      warnings.push(
        'No image is currently stored; publish will require at least one HTTPS image',
      );
  ...
  async getCategoryMetadata(
    ...
    marketplaceId: string,
    categoryId: string,
  ): Promise<VerticalCategoryMetadata> {
    ...
    const [aspects, policy] = await Promise.all([
      ...
      this.sellingMetadata
        .getCategoryPolicies(storeId, marketplaceId, normalizedCategory)
        .catch(() => null),
    ]);
  ...
  async persistCategoryMetadata(
    productId: string,
    organizationId: string | null,
    vertical: ProductVertical,
    metadata: VerticalCategoryMetadata,
  ) {
  ...
  async createFamily(input: {
    ...
    if (input.vertical === DEFAULT_PRODUCT_VERTICAL) {
      throw new BadRequestException(
        'Product families are only available for pilot non-automotive verticals',
      );
    }
    ...
    if (product.vertical && product.vertical !== input.vertical)
      throw new ConflictException(
        'Product family vertical does not match the catalog product',
      );
  ...
  async createVariant(input: {
    ...
    const validated = validateVerticalAttributes(
      family.vertical,
      input.attributes ?? {},
    );
    if (validated.errors.length)
  ...
  async createVariantMapping(input: {
    organizationId: string;
    variantId: string;
    ...
    if (!account)
      throw new NotFoundException(
        'eBay account not found in this organization',
      );
  ...
  private async assertStoreOrganization(
    storeId: string,
    organizationId: string,
  ): Promise<Store> {
    const store = await this.storeRepo.findOneBy({ id: storeId });
    ...
    if (!account)
      throw new NotFoundException(
        `Store ${storeId} is not available in this organization`,
      );
    return store;
  }

  private async assertFeatureEnabledForNonAutomotive(
    vertical: ProductVertical,
  ) {
  ...
  private mapCondition(
    ...
    if (raw.includes('2500') || raw.includes('refurb'))
