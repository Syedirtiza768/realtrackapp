  ...
  async buildGroupPayload(
    ...
    for (const variant of variants) {
      if (!variant.sku.trim() || seenSkus.has(variant.sku.trim())) {
        throw new BadRequestException(
          'Variant SKUs must be present and unique',
        );
      }
      if (variant.vertical !== family.vertical) {
        throw new BadRequestException(
          'All variants must use the family vertical',
        );
      }
  ...
  async publishFamily(input: VariantFamilyPublishInput) {
    ...
    if (!account)
      throw new NotFoundException(
        'eBay account/store is not in this organization',
      );
    ...
    if (family.vertical === 'automotive') {
      throw new BadRequestException(
        'Item-group publishing is not available for automotive',
      );
    }
    ...
    if (!categoryId)
      throw new BadRequestException(
        'A leaf eBay category is required for the family',
      );
    if (!input.merchantLocationKey.trim())
      throw new BadRequestException('A merchant location is required');
    ...
    for (const variant of variants) {
      ...
      const item: EbayInventoryItem = {
        ...
        condition,
        availability: {
          shipToLocationAvailability: { quantity: variant.quantity ?? 0 },
        },
      };
    ...
    const groupKey = `${family.slug}-${input.marketplaceId}`
      .replace(/[^a-zA-Z0-9_-]/g, '-')
      .slice(0, 50);
    await this.inventoryApi.createOrReplaceInventoryItemGroup(
      input.storeId,
      groupKey,
      group,
    );
    const published = await this.inventoryApi.publishOfferByInventoryItemGroup(
      input.storeId,
      groupKey,
    );
