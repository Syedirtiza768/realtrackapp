...
export class BusinessIndustrialIncidentNotifications {
  ...
  async onIncident(data: { organizationId: string; incidentId?: string; productId?: string; status?: string; notifyVerticalAdministrators?: boolean }) {
    if (!data.notifyVerticalAdministrators) return;
    const members = await this.members.find({ where: { organizationId: data.organizationId } });
    await Promise.all(members.map(async (member) => {
      if (!(await this.rbac.userHasPermission(member.userId, 'business_industrial.incidents.view'))) return;
