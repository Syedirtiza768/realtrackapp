...
export default function FashionShell({ children }: { children: React.ReactNode }) {
  ...
  const links = [
    ...
    ['/fashion/listings', 'Listings', 'fashion.listings.view'],
    ...
    <div className="min-h-screen bg-slate-100 text-slate-900 dark:bg-slate-950 dark:text-slate-100">
      <header className="border-b border-slate-200 bg-white dark:border-slate-800 dark:bg-slate-900">
        <div className="mx-auto flex max-w-7xl items-center justify-between px-6 py-4">
          ...
          <button className="flex items-center gap-2 text-sm text-slate-500 hover:text-slate-900 dark:hover:text-white" onClick={async () => { await logout(); navigate('/fashion/login', { replace: true }); }}><LogOut size={16} /> Sign out</button>
        </div>
      </header>
