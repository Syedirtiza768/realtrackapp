  ...
  return (
    <main className="min-h-screen bg-slate-950 px-4 py-16 text-white">
      <form onSubmit={submit} className="mx-auto max-w-md rounded-2xl border border-pink-400/20 bg-slate-900 p-8 shadow-2xl">
        <p className={businessIndustrial ? 'text-xs font-semibold uppercase tracking-[0.3em] text-cyan-300' : automotive ? 'text-xs font-semibold uppercase tracking-[0.3em] text-blue-300' : 'text-xs font-semibold uppercase tracking-[0.3em] text-pink-300'}>{businessIndustrial ? 'Omni Core B&I' : automotive ? 'Omni Core Auto Parts' : 'Omni Core Fashion'}</p>
        ...
        <button className={businessIndustrial ? 'mt-6 w-full rounded-lg bg-cyan-600 px-4 py-2.5 font-semibold disabled:opacity-50' : 'mt-6 w-full rounded-lg bg-pink-500 px-4 py-2.5 font-semibold disabled:opacity-50'} disabled={saving} type="submit">{saving ? 'Saving password…' : 'Save password and continue'}</button>
