...
export default function BusinessIndustrialShell({ children }: { children: React.ReactNode }) {
  ...
  const links: [string, string][] = [
    ['/business-industrial', 'Overview'],
    ['/business-industrial/listings', 'Listings'],
    ['/business-industrial/import', 'Bulk import'],
    ['/business-industrial/review', 'Compliance review'],
    ['/business-industrial/stores', 'Stores'],
    ['/business-industrial/incidents', 'Incidents'],
  ];
  if (permissions.includes('business_industrial.users.manage')) links.push(['/business-industrial/users', 'Users']);

  return <div className="min-h-screen bg-slate-100 text-slate-900 dark:bg-slate-950 dark:text-slate-100">
    <header className="border-b border-slate-200 bg-white dark:border-slate-800 dark:bg-slate-900"><div className="mx-auto flex max-w-7xl items-center justify-between px-6 py-4"><div className="flex items-center gap-3"><div className="rounded-xl bg-cyan-600 p-2 text-white"><Factory size={20} /></div><div><p className="font-semibold">Omni Core Business &amp; Industrial</p><p className="text-xs text-slate-500">{user?.name || user?.email}</p></div></div><button className="flex items-center gap-2 text-sm text-slate-500 hover:text-slate-900 dark:hover:text-white" onClick={async () => { await logout(); navigate('/business-industrial/login', { replace: true }); }}><LogOut size={16} /> Sign out</button></div></header>
