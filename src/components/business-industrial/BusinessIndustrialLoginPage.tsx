...
export default function BusinessIndustrialLoginPage() {
  ...
  return (
    <div className="flex min-h-[100dvh] items-center justify-center bg-slate-50 px-4 py-6 font-sans dark:bg-slate-950 pt-[max(1.5rem,env(safe-area-inset-top))] pb-[max(1.5rem,env(safe-area-inset-bottom))]">
      <div className="w-full max-w-md">
        <div className="mb-6 text-center sm:mb-8">
          {logoSrc ? (
            <img src={toProxyUrl(logoSrc)} alt="" className="mx-auto mb-3 h-12 max-w-[12rem] object-contain sm:h-14" />
          ) : (
            <div className="mx-auto mb-3 inline-flex h-12 w-12 items-center justify-center rounded-xl bg-cyan-600 text-white sm:h-14 sm:w-14">
        ...
        <div className="rounded-xl border border-slate-200 bg-white p-5 shadow-sm dark:border-slate-700 dark:bg-slate-900 sm:p-8">
          ...
          <form onSubmit={submit} className="space-y-4">
            ...
            <button
              className="mt-2 inline-flex min-h-11 w-full items-center justify-center gap-2 rounded-lg px-4 py-2.5 text-base font-semibold text-white disabled:opacity-50 sm:text-sm"
