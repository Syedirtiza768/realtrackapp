...
export default function BusinessIndustrialImageIntakeWorkspacePage() {
  ...
  async function createDraft() {
    ...
    try {
      ...
      await refreshGroups(selectedGroup.jobId); await refreshJobs(selectedGroup.jobId); navigate('/business-industrial/listings');
    } catch (error) { setMessage(messageOf(error)); } finally { setBusy(false); }
  }
  ...
  return <div className="space-y-6">
    <div className="flex flex-wrap items-end justify-between gap-4"><div><p className="text-sm font-medium text-cyan-600">Business &amp; Industrial workspace</p><h1 className="mt-1 text-3xl font-semibold">Image intake</h1><p className="mt-2 max-w-3xl text-sm text-slate-500">Import product images, identify parts with AI, review the evidence, and move approved information into the B&amp;I catalog.</p></div><div className="flex flex-wrap gap-2"><button type="button" onClick={() => void refreshJobs(job?.id)} className="inline-flex items-center gap-2 rounded-lg border border-slate-300 bg-white px-3 py-2 text-sm font-semibold dark:border-slate-700 dark:bg-slate-900"><RefreshCw size={15} /> Refresh runs</button><button type="button" onClick={() => navigate('/business-industrial/listings')} className="inline-flex items-center gap-2 rounded-lg bg-cyan-600 px-3 py-2 text-sm font-semibold text-white"><ExternalLink size={15} /> Open catalog</button></div></div>
    {message && <div role="status" className="flex items-start justify-between gap-3 rounded-lg border border-cyan-200 bg-cyan-50 px-4 py-3 text-sm text-cyan-900 dark:border-cyan-900 dark:bg-cyan-950/30 dark:text-cyan-200"><span>{message}</span><button type="button" onClick={() => setMessage('')} aria-label="Dismiss message"><X size={16} /></button></div>}
