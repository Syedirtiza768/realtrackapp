      ...
      >
        ...
        <div className="px-5 py-3 border-b border-slate-200 dark:border-slate-800 flex items-center justify-between shrink-0">
          <div className="flex items-center gap-3">
            <h3 className="font-semibold text-slate-900 dark:text-slate-100 text-sm">Product Detail</h3>
            {data?.customLabelSku && (
              <button
                onClick={copySku}
          ...
          <div className="overflow-y-auto flex-1">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-0">
              ...
              <div className="bg-slate-950 relative group">
                {images.length > 0 ? (
                  <>
                    ...
                    {/* Thumbnail strip */}
                    {images.length > 1 && (
                      <div className="flex gap-1 p-2 overflow-x-auto bg-white/50 dark:bg-slate-900/50">
                        {images.map((url, i) => (
                          <button
                            key={i}
                  ...
                  <div className="h-52 sm:h-64 lg:h-80 flex items-center justify-center text-slate-600 dark:text-slate-700">
                    <ImageIcon size={60} />
                  </div>
                )}
              </div>
              ...
              <div className="p-4 sm:p-5 space-y-3 sm:space-y-4">
                {/* Title */}
                <h4 className="text-base sm:text-lg font-bold text-slate-900 dark:text-slate-100 leading-tight">{data.title}</h4>

                {/* Price & condition */}
                <div className="flex flex-wrap items-center gap-2 sm:gap-3">
                  {price !== null && (
                    <span className="text-2xl font-bold text-slate-900 dark:text-slate-100">${price.toFixed(2)}</span>
                  )}
                  {data.conditionId && (
