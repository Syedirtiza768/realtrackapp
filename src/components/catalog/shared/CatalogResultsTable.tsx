          ...
          <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
            {items.map((item) => (
              <tr key={item.id} className={selected.has(item.id) ? 'bg-blue-50/50 dark:bg-blue-950/10' : undefined}>
                ...
                <td className="max-w-[320px] px-3 py-3 align-top"><div className="flex gap-3"><div className="h-12 w-12 shrink-0 overflow-hidden rounded-lg bg-slate-100 dark:bg-slate-800">{item.imageUrls[0] ? <OptimizedImage src={item.imageUrls[0]} alt="" variant="thumb" className="h-full w-full object-cover" /> : <ImageOff className="m-3 text-slate-400" size={24} />}</div><div className="min-w-0"><button type="button" onClick={() => onView(item)} className="line-clamp-2 text-left font-medium text-slate-900 hover:text-blue-600 dark:text-white">{item.title || 'Untitled product'}</button><p className="mt-1 truncate text-xs text-slate-500">{item.brand || 'No brand'}{item.manualReview ? ' · review hold' : ''}</p></div></div></td>
                <td className="px-3 py-3 align-top text-xs text-slate-600 dark:text-slate-300"><div>{item.sku || '—'}</div><div className="mt-1">{item.mpn || '—'}</div></td>
                <td className="max-w-[180px] px-3 py-3 align-top text-xs text-slate-600 dark:text-slate-300"><span className="line-clamp-2">{item.categoryName || item.categoryId || '—'}</span><div className="mt-1 text-slate-400">{item.teamName || 'Unassigned'}</div></td>
                ...
                <td className="px-3 py-3 align-top"><span className={'inline-flex rounded-full px-2 py-1 text-xs font-medium ' + statusClass(item.publicationStatus === 'blocked' ? 'blocked' : item.verticalValidationStatus)}>{item.publicationStatus === 'blocked' ? 'Blocked' : item.verticalValidationStatus.replace(/_/g, ' ')}</span><div className="mt-1 text-xs text-slate-500">{item.reviewStatus || 'No review'}</div></td>
                <td className="px-3 py-3 align-top"><div className="flex max-w-[170px] flex-wrap gap-1">{item.publications.length ? item.publications.slice(0, 3).map((publication) => <a key={publication.id} href={publication.listingUrl || '#'} target="_blank" rel="noreferrer" className={'rounded-full px-2 py-1 text-[11px] ' + statusClass(publication.listingStatus)}>{publication.storeName}: {publication.listingStatus}</a>) : <span className="text-xs text-slate-500">Unpublished</span>}</div></td>
                <td className="px-3 py-3 align-top"><div className="flex justify-end gap-1"><button type="button" onClick={() => onView(item)} title="Quick view" className="rounded-lg p-2 text-slate-500 hover:bg-slate-100 hover:text-slate-900 dark:hover:bg-slate-800 dark:hover:text-white"><Eye size={16} /></button><button type="button" onClick={() => onEdit(item)} title="Open full editor" className="rounded-lg p-2 text-slate-500 hover:bg-slate-100 hover:text-slate-900 dark:hover:bg-slate-800 dark:hover:text-white"><Edit3 size={16} /></button>{canPublish && item.publicationStatus !== 'blocked' ? <button type="button" onClick={() => onPublish(item)} title="Publish" className="rounded-lg p-2 text-slate-500 hover:bg-emerald-50 hover:text-emerald-700 dark:hover:bg-emerald-950/30 dark:hover:text-emerald-300"><Send size={16} /></button> : null}</div></td>
              </tr>
            ))}
          </tbody>
        </table>
