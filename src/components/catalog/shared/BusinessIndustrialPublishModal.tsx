...
export default function BusinessIndustrialPublishModal({ open, item, listingIds, accounts, organizationId, onClose, onSubmitted }: Props) {
  ...
  return (
    <div className="fixed inset-0 z-[55] flex items-end justify-center bg-slate-950/60 sm:items-center sm:p-4" role="dialog" aria-modal="true" aria-label="Publish Business and Industrial listings" onClick={onClose}>
      <div className="max-h-[min(92dvh,calc(100dvh-env(safe-area-inset-bottom,0px)))] w-full max-w-2xl overflow-y-auto overscroll-contain rounded-t-2xl bg-white p-4 pb-[max(1.5rem,env(safe-area-inset-bottom))] shadow-2xl dark:bg-slate-900 sm:rounded-2xl sm:p-6" onClick={(event) => event.stopPropagation()}>
        ...
        <div className="mt-6 flex flex-col-reverse gap-2 border-t border-slate-200 pt-4 dark:border-slate-700 sm:flex-row sm:justify-end"><button type="button" onClick={onClose} className="min-h-11 rounded-lg border border-slate-300 px-4 py-2 text-sm font-medium text-slate-700 hover:bg-slate-50 dark:border-slate-600 dark:text-slate-200 dark:hover:bg-slate-800">Cancel</button><button type="button" disabled={mode === 'single' ? !canPublishSingle : !canPublishBulk} onClick={() => void publish()} className="inline-flex min-h-11 items-center justify-center gap-2 rounded-lg bg-emerald-600 px-4 py-2 text-sm font-semibold text-white hover:bg-emerald-700 disabled:opacity-50">{busy === 'publish' ? <Loader2 size={15} className="animate-spin" /> : <Send size={15} />} {busy === 'publish' ? 'Submitting…' : mode === 'single' ? `Publish to ${selectedAccounts.length || '…'} store${selectedAccounts.length !== 1 ? 's' : ''}` : `Publish ${listingIds.length.toLocaleString()} listings`}</button></div>
      </div>
    </div>
  );
}
