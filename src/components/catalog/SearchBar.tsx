  ...
  return (
    <div className="relative w-full">
      ...
      {showDropdown && (
        <div
          ref={dropdownRef}
          className="absolute z-50 top-full left-0 right-0 mt-1 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-xl shadow-2xl shadow-black/40 overflow-hidden max-h-80 overflow-y-auto"
        >
              ...
              {recentSearches.map((r, i) => (
                <button
                  ...
                  className={`w-full px-3 py-2 flex items-center gap-2 text-sm text-left transition-colors ${
                    highlightIdx === i ? 'bg-slate-800 text-slate-900 dark:text-slate-100' : 'text-slate-400 dark:text-slate-400 hover:bg-slate-100/60 dark:bg-slate-800/60'
                  }`}
                >
                  <Clock size={13} className="text-slate-500 dark:text-slate-600 shrink-0" />
                  {r}
                </button>
              ))}
            </div>
                ...
                return (
                  <button
                    ...
                    className={`w-full px-3 py-2 flex items-center gap-3 text-sm text-left transition-colors ${
                      highlightIdx === i ? 'bg-slate-800 text-slate-900 dark:text-slate-100' : 'text-slate-500 dark:text-slate-300 hover:bg-slate-100/60 dark:bg-slate-800/60'
                    }`}
                  >
                    ...
                    <span className="text-[10px] font-medium uppercase text-slate-500 dark:text-slate-600 shrink-0">
                      {TYPE_LABELS[s.type] ?? s.type}
                    </span>
                    {s.count != null && (
                      <span className="text-[10px] text-slate-500 dark:text-slate-600 shrink-0 tabular-nums">
