          ...
          <div className="space-y-3">
            ...
            {submittedVin && (
              <p className="text-xs text-slate-500">
                Searching listings for VIN <span className="font-mono text-slate-300">{submittedVin}</span>
              </p>
            )}
          </div>
        </CardHeader>
        <CardContent className="p-0">
          ...
          {!isLoading && data && (
            <div className="p-4 space-y-4">
              <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2 border border-slate-800 rounded-lg bg-slate-900/60 px-3 py-3 sm:px-4 sm:py-3">
                <div>
                ...
                <div className="text-xs text-slate-400 flex flex-wrap gap-3">
                  <span>
                    <span className="font-semibold text-slate-200">{data.totalListings}</span>{' '}
                    listing{data.totalListings === 1 ? '' : 's'}
                  </span>
                  <span className="hidden sm:inline">•</span>
                  <span>
                    <span className="font-semibold text-slate-200">{data.totalFitments}</span>{' '}
                        ...
                        <tr key={row.id} className="hover:bg-slate-800/50 transition-colors">
                          <td className="p-3 sm:p-4">
                            <div className="flex items-center gap-3">
                              {row.itemPhotoUrl ? (
                                ...
                                <div className="w-12 h-12 rounded-md bg-slate-900 border border-slate-800 flex items-center justify-center text-[10px] text-slate-500 shrink-0">
                                  No Image
                                </div>
                              )}
                              <div className="min-w-0">
                          ...
                          <td className="p-3 sm:p-4">
                            <div className="text-slate-200">
                              {row.cBrand || <span className="text-slate-500">—</span>}
                            </div>
                            <div className="text-xs text-slate-500">
                              MPN:{' '}
                              <span className="font-mono">
                          ...
                          <td className="p-3 sm:p-4">
                            <div className="text-slate-200">
                              {row.categoryName || <span className="text-slate-500">—</span>}
                            </div>
                            <div className="text-xs text-slate-500">
                              ID: {row.categoryId || '—'}
                            </div>
                          ...
                          <td className="p-3 sm:p-4">
                            <div className="text-xs inline-flex px-2 py-0.5 rounded-full bg-slate-800 text-slate-300">
                              {row.conditionId || '—'}
                            </div>
                          </td>
                        </tr>
