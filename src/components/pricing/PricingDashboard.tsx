    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        ...
        <button
          ...
          className="px-3 py-1.5 text-xs font-medium rounded-md bg-slate-200 dark:bg-slate-700 hover:bg-slate-300 dark:hover:bg-slate-600 disabled:opacity-50 flex items-center gap-1"
        >
      ...
      <div className="grid gap-3 grid-cols-2 lg:grid-cols-4">
        <Card>
          <CardHeader className="pb-2">
            <CardTitle className="text-xs font-medium text-slate-400 dark:text-slate-400">Market Avg</CardTitle>
          </CardHeader>
          <CardContent>
        ...
        <Card>
          <CardHeader className="pb-2">
            <CardTitle className="text-xs font-medium text-slate-400 dark:text-slate-400">Price Range</CardTitle>
          </CardHeader>
          <CardContent>
        ...
        <Card>
          <CardHeader className="pb-2">
            <CardTitle className="text-xs font-medium text-slate-400 dark:text-slate-400">Your Price</CardTitle>
          </CardHeader>
          <CardContent>
        ...
        <Card>
          <CardHeader className="pb-2">
            <CardTitle className="text-xs font-medium text-slate-400 dark:text-slate-400">Competitors</CardTitle>
          </CardHeader>
          <CardContent>
            <div className="text-lg font-bold">{competitors.length}</div>
            <p className="text-xs text-slate-400 dark:text-slate-400 mt-1">tracked (30 days)</p>
          </CardContent>
        </Card>
      </div>
          ...
          <CardContent>
            <ul className="space-y-1">
              {snapshot.marketInsights.map((insight: string, i: number) => (
                <li key={i} className="text-sm text-slate-500 dark:text-slate-300 flex items-start gap-2">
        ...
        <CardContent className="space-y-3">
          {!showSuggestion ? (
            ...
            <div className="space-y-3">
              ...
              <p className="text-sm text-slate-500 dark:text-slate-300">{suggestion.reasoning}</p>

              <div className="grid grid-cols-3 gap-2 text-xs text-slate-400 dark:text-slate-400">
                <div>
                  <span className="block font-medium text-slate-500 dark:text-slate-300">Confidence</span>
                  {(suggestion.confidence * 100).toFixed(0)}%
                </div>
                <div>
                  <span className="block font-medium text-slate-500 dark:text-slate-300">Margin</span>
                  {suggestion.marginPercent.toFixed(1)}%
                </div>
                <div>
                  <span className="block font-medium text-slate-500 dark:text-slate-300">Range</span>
              ...
              <div className="flex gap-2 pt-2">
                ...
                <button
                  ...
                  className="px-3 py-2 text-sm font-medium rounded-md bg-slate-200 dark:bg-slate-700 hover:bg-slate-300 dark:hover:bg-slate-600"
                >
                  Dismiss
                </button>
              </div>
                ...
                <div className="rounded-md bg-slate-100 dark:bg-slate-800 p-3 text-xs space-y-1">
                  {repriceMutation.data.results.map((r: { offerId: string; storeName: string; oldPrice: number | null; newPrice: number; action: string; error?: string }) => (
                    <div key={r.offerId} className="flex items-center gap-2">
                      ...
                      <span className="text-slate-500 dark:text-slate-300">{r.storeName}:</span>
                      {r.action === 'repriced' ? (
                        ...
                        <span className="text-slate-400 dark:text-slate-500">{r.action}</span>
                      )}
                    </div>
                  ))}
                </div>
          ...
          <CardContent>
            <div className="max-h-64 overflow-y-auto">
              <table className="w-full text-xs">
                <thead className="text-slate-400 dark:text-slate-400 border-b border-slate-200 dark:border-slate-700">
                  <tr>
                ...
                <tbody className="divide-y divide-slate-200 dark:divide-slate-800">
                  {competitors.slice(0, 20).map((c: CompetitorPrice) => (
                    <tr key={c.id} className="text-slate-500 dark:text-slate-300">
                      <td className="py-1.5 pr-2 truncate max-w-[120px]">{c.seller ?? '—'}</td>
                      <td className="py-1.5 px-2 text-right font-mono">
                        {fmtUSD(c.price)}
                      </td>
                      <td className="py-1.5 px-2">{c.condition ?? '—'}</td>
                      <td className="py-1.5 pl-2 text-slate-400 dark:text-slate-500">
