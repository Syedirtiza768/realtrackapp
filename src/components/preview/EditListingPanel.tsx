...
export default function EditListingPanel({ listing, onSave, onCancel }: EditListingPanelProps) {
  ...
  const inputClasses = 'w-full bg-slate-900 border border-slate-700 rounded-lg px-3 py-2 text-sm text-slate-200 placeholder-slate-500 focus:outline-none focus:border-blue-500';
  ...
  function SectionHeader({ title, sectionKey, count }: { title: string; sectionKey: string; count?: number }) {
    ...
    return (
      <button
        ...
        className="w-full flex items-center justify-between py-2 px-1 text-sm font-semibold text-slate-200 hover:text-white transition-colors"
      >
  ...
  return (
    <div className="h-full flex flex-col bg-slate-800 border-l border-slate-700">
      {/* Header */}
      <div className="flex items-center justify-between px-4 py-3 border-b border-slate-700 bg-slate-800/80 backdrop-blur-sm sticky top-0 z-10">
        <h2 className="text-sm font-bold text-slate-100">Edit Listing</h2>
        ...
        {expandedSections.basic && (
          <div className="space-y-3 pb-3">
            <div>
              ...
              <p className="text-xs text-slate-500 mt-0.5">{draft.title.length}/80 characters</p>
            </div>
          ...
          <div className="space-y-3 pb-3">
            ...
            <div className="mt-2">
              <div className="flex items-center justify-between mb-2">
                <span className="text-xs font-medium text-slate-400">Custom Specifics</span>
              ...
              {draft.itemSpecifics.map((spec, i) => (
                <div key={i} className="flex items-center gap-2 mb-2">
                  ...
                  />
                  <button onClick={() => removeSpecific(i)} className="p-1.5 text-slate-500 hover:text-red-400">
            ...
            {draft.imageUrls.map((url, i) => (
              <div key={i} className="flex items-center gap-2">
                ...
                <input
                  ...
                  placeholder="https://..."
                />
                <button onClick={() => removeImageUrl(i)} className="p-1.5 text-slate-500 hover:text-red-400">
                  <Trash2 className="w-3.5 h-3.5" />
                </button>
              </div>
            ))}
          ...
          <div className="space-y-2 pb-3">
            {draft.compatibility.map((compat, i) => (
              <div key={i} className="flex items-center gap-2">
                ...
                <button onClick={() => removeCompat(i)} className="p-1.5 text-slate-500 hover:text-red-400">
                  <Trash2 className="w-3.5 h-3.5" />
                </button>
              </div>
            ))}
