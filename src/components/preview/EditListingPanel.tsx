...
export default function EditListingPanel({ listing, onSave, onCancel }: EditListingPanelProps) {
  ...
  const labelClasses = 'block text-xs font-medium text-slate-400 dark:text-slate-400 mb-1';
    ...
    <div className="h-full flex flex-col bg-slate-800 border-l border-slate-200 dark:border-slate-700">
      ...
      <div className="flex items-center justify-between px-4 py-3 border-b border-slate-200 dark:border-slate-700 bg-slate-100/80 dark:bg-slate-800/80 backdrop-blur-sm sticky top-0 z-10">
        ...
        <div className="flex items-center gap-2">
          ...
          <button
            onClick={onCancel}
            className="p-1.5 text-slate-400 dark:text-slate-400 hover:text-slate-600 dark:text-slate-200 transition-colors"
          >
            <X className="w-4 h-4" />
          </button>
        </div>
        ...
        {expandedSections.basic && (
          <div className="space-y-3 pb-3">
            <div>
              ...
              <p className="text-xs text-slate-400 dark:text-slate-500 mt-0.5">{draft.title.length}/80 characters</p>
            </div>
          ...
          <div className="space-y-3 pb-3">
            ...
            <div className="mt-2">
              <div className="flex items-center justify-between mb-2">
                <span className="text-xs font-medium text-slate-400 dark:text-slate-400">Custom Specifics</span>
              ...
              {draft.itemSpecifics.map((spec, i) => (
                <div key={i} className="flex items-center gap-2 mb-2">
                  ...
                  />
                  <button onClick={() => removeSpecific(i)} className="p-1.5 text-slate-400 dark:text-slate-500 hover:text-red-400">
            ...
            {draft.imageUrls.map((url, i) => (
              <div key={i} className="flex items-center gap-2">
                ...
                <input
                  ...
                  placeholder="https://..."
                />
                <button onClick={() => removeImageUrl(i)} className="p-1.5 text-slate-400 dark:text-slate-500 hover:text-red-400">
                  <Trash2 className="w-3.5 h-3.5" />
                </button>
              </div>
            ))}
          ...
          <div className="space-y-2 pb-3">
            {draft.compatibility.map((compat, i) => (
              <div key={i} className="flex items-center gap-2">
                ...
                <button onClick={() => removeCompat(i)} className="p-1.5 text-slate-400 dark:text-slate-500 hover:text-red-400">
                  <Trash2 className="w-3.5 h-3.5" />
                </button>
              </div>
            ))}
