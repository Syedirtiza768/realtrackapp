      ...
      <div className="space-y-6">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-slate-500 dark:text-slate-400">Upload a pipeline output CSV to preview exactly how listings will appear on eBay</p>
          </div>
        </div>
          ...
          <CardContent>
            ...
            >
              <input
                ...
                className="hidden"
              />
              <Eye className="h-12 w-12 text-slate-500 dark:text-slate-400 mb-4" />
    ...
    <div className="space-y-4">
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-3">
          ...
          <button
            onClick={handleReset}
            className="text-sm text-slate-500 dark:text-slate-400 hover:text-slate-600 dark:text-slate-200 flex items-center gap-1.5 transition-colors"
          >
            <X className="w-4 h-4" />
            Clear & Upload New
          </button>
          ...
          <div className="flex items-center gap-4">
            ...
            <div className="flex-1 relative">
              ...
              {searchQuery && (
                <button
                  ...
                  className="absolute right-3 top-1/2 -translate-y-1/2 text-slate-500 dark:text-slate-400 hover:text-slate-600 dark:text-slate-200"
            ...
            <div className="flex items-center border border-slate-700 rounded-lg overflow-hidden">
              <button
                ...
                className={`px-3 py-2 text-xs font-medium transition-colors ${viewMode === 'single' ? 'bg-blue-600 text-white' : 'text-slate-500 dark:text-slate-400 hover:text-slate-600 dark:text-slate-200'}`}
              >
                Single
              </button>
              <button
                onClick={() => setViewMode('grid')}
                className={`px-3 py-2 text-xs font-medium transition-colors flex items-center gap-1 ${viewMode === 'grid' ? 'bg-blue-600 text-white' : 'text-slate-500 dark:text-slate-400 hover:text-slate-600 dark:text-slate-200'}`}
              >
                <LayoutGrid className="w-3.5 h-3.5" /> Grid
              </button>
              <button
                onClick={() => setViewMode('list')}
                className={`px-3 py-2 text-xs font-medium transition-colors flex items-center gap-1 ${viewMode === 'list' ? 'bg-blue-600 text-white' : 'text-slate-500 dark:text-slate-400 hover:text-slate-600 dark:text-slate-200'}`}
              >
            ...
            {viewMode === 'single' && (
              <div className="flex items-center gap-2">
                <button
                  ...
                  className={`flex items-center gap-1.5 px-3 py-1.5 text-xs font-medium rounded-lg border transition-colors ${
                    editingIndex !== null
                      ...
                      : 'border-slate-700 text-slate-500 dark:text-slate-400 hover:text-slate-600 dark:text-slate-200 hover:border-slate-500'
                  }`}
                >
                ...
                <button
                  ...
                  className="p-1.5 rounded-lg border border-slate-700 text-slate-500 dark:text-slate-400 hover:text-slate-600 dark:text-slate-200 hover:border-slate-500 disabled:opacity-30 disabled:cursor-not-allowed transition-colors"
                >
        ...
        currentListing ? (
          ...
          <Card>
            <CardContent className="py-16 text-center space-y-2">
              {searchQuery ? (
                <>
                  <p className="text-slate-500 dark:text-slate-400">No listings match <span className="text-slate-600 dark:text-slate-200">&ldquo;{searchQuery}&rdquo;</span>.</p>
        ...
        <div className="rounded-xl border border-slate-700 overflow-hidden">
          ...
          {filteredListings.map((listing, i) => (
            ...
            >
              ...
              {/* SKU */}
              <div className="text-xs text-slate-500 dark:text-slate-400 truncate pr-2">{listing.customLabel || '—'}</div>
              {/* Price */}
              <div className="text-sm font-semibold text-slate-900 dark:text-slate-100">
                US ${parseFloat(listing.price || '0').toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
              </div>
              {/* Condition */}
              <div className="text-xs text-slate-500 dark:text-slate-400 truncate">{listing.conditionLabel || 'Used'}</div>
              {/* Brand */}
              <div className="text-xs text-slate-500 dark:text-slate-400 truncate">{listing.brand || '—'}</div>
              {/* Fitments */}
              <div className="text-xs text-slate-500 dark:text-slate-400">{listing.compatibility.length > 0 ? listing.compatibility.length : '—'}</div>
            </div>
          ))}
        </div>
      )}
