...
export async function fetchWithAuth<T = unknown>(
  ...
  if (res.status === 401) {
    ...
    if (!window.location.pathname.startsWith('/login') && !window.location.pathname.startsWith('/fashion/login')) {
      window.location.href = window.location.pathname.startsWith('/fashion') ? '/fashion/login' : '/login';
    }
