...
- **Catalog bulk publish — live per-listing progress + batch summary:** `PublishProgressPanel` bulk mode is now listing-centric instead of store-centric. The panel drives publish in five-listing backend requests (`BULK_REQUEST_SIZE = 5`), so each listing's **name** appears live as it publishes with a real-time "X of N listings" counter and progress bar without consuming one authenticated request per listing. Each listing resolves independently to `success` / `partial` (some stores failed) / `failed`, showing its error inline. A **"View summary"** section expands after completion with totals (Listings / Published / Partial / Failed), a per-store success/fail breakdown, and a "Needs attention" list of every failed or partial listing with its error. Listing names are sourced from the loaded catalog rows via a new `PublishJob.listingNames` map built in `CatalogManager.handlePublishStart`. Fully responsive (truncation, scrollable listing list, `grid-cols-2 sm:grid-cols-4` summary tiles) and theme-aware. Single-listing publish is unchanged (still per-store).
...
- **Catalog bulk publish transient recovery and throttle safety:** `PublishProgressPanel` now sends five listing IDs per backend request instead of consuming one authenticated HTTP request per listing, preventing large page-by-page publishes from exhausting the 1,000-request/hour user bucket. It retries only failed store/listing pairs for transient SellerPundit `Product not found`, Core Inventory 500, timeout, rate-limit, and `ThrottlerException: Too Many Requests` responses, with longer exponential backoff for throttling. `CatalogManager` keys the progress panel by publish-job ID so a targeted retry cannot inherit the previous job's listing state or display a stale summary.
