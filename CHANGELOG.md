...
- **Catalog bulk publish — live per-listing progress + batch summary:** `PublishProgressPanel` bulk mode is now listing-centric instead of store-centric. Rather than one blocking `/publish-by-listings` call that only reported per-store aggregates, the panel now drives the publish client-side in concurrent chunks (`BULK_CONCURRENCY = 5`, matching the backend), so each listing's **name** appears live as it publishes with a real-time "X of N listings" counter and progress bar. Each listing resolves independently to `success` / `partial` (some stores failed) / `failed`, showing its error inline. A **"View summary"** section expands after completion with totals (Listings / Published / Partial / Failed), a per-store success/fail breakdown, and a "Needs attention" list of every failed or partial listing with its error. Listing names are sourced from the loaded catalog rows via a new `PublishJob.listingNames` map built in `CatalogManager.handlePublishStart`. Fully responsive (truncation, scrollable listing list, `grid-cols-2 sm:grid-cols-4` summary tiles) and theme-aware. Single-listing publish is unchanged (still per-store). Publish semantics and the backend are untouched — this is a frontend orchestration/UI change only.
