...
- **eBay compatibility/fitment not published separately from description:** Listings with vehicle fitment visible in the description HTML were not always sending structured `product_compatibility` data to eBay. `ListingBuilderService` now falls back from `catalog_products.fitment_data` to `fitment_rows`, and `EbayPublishService.enrichPublishRequest` derives structured compatibility from the catalog product for legacy `publish-by-listing-id` / `channels` queue paths. `fitment-mvl.util.ts` handles both `MvlStatus` and `validationStatus` rejection flags, `yearStart/yearEnd` ranges, and mixed-case field names.
