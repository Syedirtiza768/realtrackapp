...
### Fixed
- **Pipeline title sanitization — VIN/part number/gibberish stripping:** Pipeline listings (e.g. job `572e96dd`) had titles like `2005-2015 AUDI Q7 Wauay54l98d068586, Audi, Q7, Ultrasonic Sensor 8E0054635A OEM` containing VIN numbers, OEM part numbers, and duplicate make/model. Root cause: the source spreadsheet's raw data (including VINs) was passed through to the AI enrichment prompt, which faithfully included it in the title. No existing guard stripped VINs or part numbers from titles. Fix: (1) Added `sanitizeTitle()` to `listing-guards.ts` — deterministic regex-based stripping of VIN blocks (17-char alphanumeric + ", Make, Model, "), part numbers before condition words (OEM/Used/New), and trailing part numbers at end of title. Part number patterns require ≥1 digit to avoid stripping English words like "Black"/"Chrome". (2) Integrated into `applyListingGuards()` as `TITLE_SANITIZED` fix, running before condition-strip and trim. (3) Updated all 3 enrichment prompt templates and compliance service title prompt with explicit rule: NEVER include VIN numbers, raw OEM part numbers, or duplicate make/model in the title. (4) Added VIN to `TITLE_FORBIDDEN_PATTERNS` in `ebay-compliance.service.ts`. (5) Added `POST /catalog-products/sanitize-titles` endpoint for retroactive bulk cleanup. (6) Production: cleaned all 766 titles for job `572e96dd` (766 listing_records + 766 catalog_products, 0 remaining VINs).
